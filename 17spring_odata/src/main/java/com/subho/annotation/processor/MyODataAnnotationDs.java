package com.subho.annotation.processor;
...
public class MyODataAnnotationDs implements MyODataSource {
	private static final org.slf4j.Logger log = LoggerFactory.getLogger(MyODataAnnotationDs.class);
			...
			for (Class<?> clz : annotatedClasses) {
				...
				if (entitySetName != null) {
					...
					throw new ODataException(
							"Found not annotated class during DataStore initilization of type: " + clz.getName());
				}
			}
	...
	public Object readRelatedData(EdmEntitySet sourceEntitySet, Object sourceData, EdmEntitySet targetEntitySet,
			...
			throws ODataNotImplementedException, ODataNotFoundException, EdmException, ODataApplicationException {
		log.debug(
				"MyODataAnnotationDs readRelatedData with sourceEntitySet sourceData targetEntitySet targetKeys called");

		DataStore<?> sourceStore = dataStores.get(sourceEntitySet.getName());
		DataStore<?> targetStore = dataStores.get(targetEntitySet.getName());

		AnnotatedNavInfo navInfo = ANNOTATION_HELPER.getCommonNavigationInfo(sourceStore.getDataTypeClass(),
				targetStore.getDataTypeClass());
	...
	public void writeRelation(EdmEntitySet sourceEntitySet, Object sourceData, EdmEntitySet targetEntitySet,
		...
		// this.dataSourceDelegate.writeRelation(sourceEntitySet, sourceData,
		// targetEntitySet, targetKeys);
	}
